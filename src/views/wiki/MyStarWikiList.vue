...
const getList = async () => {
  try {
    ...
    const response = await axios.get(`http://${wikiAddress}/api/wikistar/read/my/${userId}`, {
      headers: {
    ...
    for (let i = startIndex; i < endIndex; i++) {
      const res2 = await axios.get(`http://${wikiAddress}/api/wikilog/readall/${cache.value[i].wiki.wikiId}/${userId}`, {
        headers: {
          Authorization: authToken,
        },
      ...
      <VCard>
        ...
        >
          ...
          <option
            ...
            :key="idx"
            :value="item.wiki.category.id"
          >
            {{ item.wiki.category.name }}
          </option>
        </select>
        <br />
        <br />
        ...
        >
          ...
          >
            <tr height="40">
              ...
              <GetName :userId='item.wiki.userId'/>
                <!-- {{ item.user.username }} -->
              </td>
              <td
                width="20%"
