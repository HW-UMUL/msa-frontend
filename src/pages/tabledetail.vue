...
const tableProfileAddress = inject('tableProfileAddress')
...
async function getTable(){
  ...
  const response = await fetch(
      `http://${tableAddress}/api/table/read/${props.tableId}/${userId}`,
      {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
...
async function getTableUsers(){
  ...
  const userId = await getUserId()
  const response = await fetch(
      `http://${tableAddress}/api/tableuser/readuser/${props.tableId}/${userId}`,
      {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
...
async function addTableUsers(){
  ...
  await Promise.all(inviteUsers.value.map(async function(user) {
    ...
    formData.userId.push(getUser.userId);
  }));
  ...
  const userId = await getUserId()
  const response = await fetch(
    `http://${tableAddress}/api/tableuser/create/${props.tableId}/${userId}`,
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
...
async function addAdminUser(){
  const userId = await getUserId()
  const response = await fetch(
    `http://${tableAddress}/api/tableuser/admin/add/${props.tableId}/${userId}`,
    {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
...
async function removeAdminUser(){
  const userId = await getUserId()
  const response = await fetch(
    `http://${tableAddress}/api/tableuser/admin/remove/${props.tableId}/${userId}`,
    {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
...
async function updateTable(){
  const userId = await getUserId()
  const response = await fetch(
    `http://${tableAddress}/api/table/update/${props.tableId}/${userId}`,
    {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
...
const response = await fetch(
    `http://${postAddress}/api/post/read/table/${props.tableId}/${userId}`,
   {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
...
async function sendFile() {
  const userId = await getUserId()
    ...
    // FormData를 서버로 전송
    axios.put(`http://${tableAddress}/api/table/update/${props.tableId}/image/${userId}`, formData, {
        headers: {
    ...
    >
      ...
      <div class="d-flex">
        <div class="table-image">
          ...
          <img @click="tableImageDialog=true" v-if="table.profile" class="table-propile-img" :src="tableProfileAddress + table.profile.storeFileName"/>          
        </div>
        <div class="d-flex flex-column" >
          ...
          <div class="d-flex mt-auto" style="margin-left:6%">
            ...
            <VBtn v-if="changePostWiki"
            @click="createTablePost"
            >
              Create Post
            </VBtn>
            <VBtn v-if="!changePostWiki"
            @click="createTableWiki"
            >
              Create Wiki
            </VBtn>
          ...
          <VDivider/>
            <div
              @click="moveTable(item.table.id)" class="d-flex">
                ...
                <img v-if="item.table.profile"  class="my-table-img" :src="tableProfileAddress + item.table.profile.storeFileName"/>          
  ...
  <!-- Dialog -->
            ...
            <VDialog v-model="userManageDialog" max-width="50%">
              <VCard>
                ...
                <VCardText>
                  <form @submit.prevent="addTableUsers">
