...
async function getTableUserIds(userIdsData){
  const userId = await getUserId()
  const response = await fetch(
    `http://${tableAddress}/api/tableuser/teams/${props.tableId}/${userId}`,
    {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
  ...
  if(!response.ok){
    ...
    userIds.forEach(function(userId) {
      userIdsData.value.userIds.push(userId)
    })
  }
}
...
async function submitForm() {
  ...
  await getTableUserIds(formData)
  ...
  // POST 요청 보내기
  axios
    .post(`http://${wikiAddress}/api/wiki/create/${props.tableId}/${userId}`, data, { headers })
    .then(response => {
