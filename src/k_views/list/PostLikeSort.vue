...
async function getLikes(postId) {
  try {
    const userId = await getUserId()
    const response = await fetch(`http://${postAddress}/api/postlike/read/${postId}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${auth}`
...
async function checkLike(postId){
  const userId = await getUserId()
  const response = await fetch(
    `http://${postAddress}/api/postlike/check/${postId}/${userId}`,
    {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
    ...
    <tbody>
      <tr v-for="(item, index) in posts.slice(0,5)" :key="index">
        <td class="text-center">
          <VIconBtn @click="clickpostmodal(item.postId)" style="cursor: pointer;">
            {{ item.postTitle }}
          </VIconBtn>
        </td>
        <td class="text-center">
          <VIconBtn @click="checkLike(item.postId)" style="cursor: pointer;">
