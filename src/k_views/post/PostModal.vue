...
async function readPost() {
  ...
  const response = await fetch(`http://${postAddress}/api/post/read/${props.post.postId}/${userId}`, {
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${auth}`,
    },
...
async function getLikes() {
  const response = await fetch(`http://${postAddress}/api/postlike/read/${props.post.postId}`, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${auth}`,
...
async function getStars() {
  const response = await fetch(`http://${postAddress}/api/poststar/read/${props.post.postId}`, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${auth}`,
...
async function gettags() {
  const response = await fetch(`http://${postAddress}/api/tagpost/read/${props.post.postId}`, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${auth}`,
...
async function getUserInfo(userId) {
  const response = await fetch(`http://${userAddress}/api/whoAreYou/${userId}`, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${auth}`,
    ...
    <VCardText>
      <VCol class="mb-2">
        <VCol class="post-details">
          ...
          <VCardActions>
            <VBtn @click="isReplyDetails = !isReplyDetails"> 댓글 </VBtn>
            <VSpacer />
            <VBtn
              icon
              size="small"
